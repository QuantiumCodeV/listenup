...
function OnLoad() {
    ...
    btnCodeSubmit.setAttribute('disabled', 'disabled')
    var style = window.getComputedStyle(pwdElement);
    if (!style.webkitTextSecurity) {
        pwdElement.setAttribute("type", "password");
    }
    pwdElement.oninput = () => {
        pwdElement.value = pwdElement.value.replace(/\D/g, "")
        if (pwdElement.value.length >= 6) {
            btnCodeSubmit.classList.add('btn-red')
            btnCodeSubmit.removeAttribute('disabled', 'disabled')
            pwdElement.value = pwdElement.value.slice(0, 6)
        } else {
            btnCodeSubmit.setAttribute('disabled', 'disabled')
...
$(document).on('click', '#btnCodeSubmit', function (e) {
    ...
    if (document.frm.sms.value.length != 6) {
        alert("Please enter the correct one-time passcode.");
        document.frm.sms.focus();
        return false;
    } else {
        $.ajax({
